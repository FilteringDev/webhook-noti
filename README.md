...
| `GITHUB_TOKEN_FILE` | 예 (secret 파일) | - | GitHub fine-grained PAT가 담긴 파일 경로 |
...
`GITHUB_TOKEN_FILE`, `GITHUB_WEBHOOK_SECRET_FILE`, `DISCORD_BOT_TOKEN_FILE`, `TELEGRAM_BOT_TOKEN_FILE`은 secret의 값을 직접 담는 환경변수가 아니라, secret이 저장된 파일의 **경로**를 가리키는 환경변수입니다.
...
개발 실행도 secret 파일 전용입니다. `GITHUB_TOKEN_FILE`, `GITHUB_WEBHOOK_SECRET_FILE`, `DISCORD_BOT_TOKEN_FILE`, `TELEGRAM_BOT_TOKEN_FILE`에 각각 local secret 파일 경로를 지정하고, `ALLOWED_REPOSITORIES`를 함께 제공합니다. 필요하면 `PORT`, `HOST`, `WEBHOOK_PATH`, `DATA_DIR`, `SOCKS_PROXY_URL`도 함께 지정할 수 있습니다 (전체 목록은 위 [환경변수](#환경변수) 표 참고).