...
async function Bootstrap(): Promise<void> {
  try {
    ...
    if (Config.DiscordToken !== undefined) Notifiers.set('discord', CreateDiscord(Config.DiscordToken, Database, () => Installations.List(), SocksBridge?.Url))
    ...
    async function ProcessRelease(ReleaseValue: Release, ReleaseKey: string): Promise<void> {
      ...
      try {
        ...
        await WaitForPurge(
          ...
          undefined,
          undefined,
          (Progress) => Logger.info({ message: PurgeProgressMessage(Progress), ReleaseKey, Repository, ...Progress })
        )
    ...
    process.on('SIGTERM', () => Server.close(() => {
      ...
      Poller.Stop()
      Installations.Stop()
      for (const Notifier of Notifiers.values()) Notifier.Stop?.()
      Database.Close()
